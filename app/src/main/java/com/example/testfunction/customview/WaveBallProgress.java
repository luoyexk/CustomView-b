...
public class WaveBallProgress extends View {
     ...
     * 波浪颜色
     */
    private int mWaveColor = 0xaa30b44e;
    /**
     * 波浪A的偏移
     */
    ...
    public void startProgress(int progress, long duration, long delay) {
        // 波纹停止时间,当progress为100时设置为1秒,否则60秒停止
        if (progress == MAX_PROGRESS) {
            if (mWaveStopAnimator != null) {
                mWaveStopAnimator.setDuration(1000);
            }
        }else {
            mWaveStopAnimator.setDuration(60 * DateUtils.SECOND_IN_MILLIS);
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (w > 0 && h > 0) {
            edge = new RectF(0, 0, getWidth(), getHeight());
            /**
             * 根据宽高初始化波浪的一些参数
            ...
            mWaveSpeedA = w / 40;
            mWaveSpeedB = w / 45;
            mWaveHeightA = dip2px(getContext(), 10);
            mWaveHeightB = dip2px(getContext(), 5);
    ...
    private void initStopAnimator(final int waveHeightA, final int waveHeightB) {
        ...
        mWaveStopAnimator.setDuration(60000);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (getHeight() > 0 && getWidth() > 0) {
            ...
            canvas.drawArc(edge, 0, 360, false, circlePaint);
            ...
            if (isWaveMoving) {
                 ...
                 */
                float height = (1 - mProgress / MAX_PROGRESS) * getHeight();
