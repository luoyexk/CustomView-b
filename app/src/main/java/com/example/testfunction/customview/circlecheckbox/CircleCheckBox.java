package com.example.testfunction.customview.circlecheckbox;
...
public class CircleCheckBox extends MaterialCheckBox {
    ...
    private int mTickColor, mBorderColor, mBackgroundColor, mBackgroundColorChecked;
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mBackgroundColorChecked = a.getColor(R.styleable.CircleCheckBox_background_color_checked, Color.parseColor("#32bc43"));
        } finally {
            a.recycle();
        }
...
//        mRectF.set(paddingLeft + mBorderWidth, paddingTop + mBorderWidth, mWidth - mBorderWidth, mHeight - mBorderWidth);
        mRectF.set(paddingLeft + mBorderWidth, paddingTop + mBorderWidth, width - paddingRight - mBorderWidth, height - paddingBottom - mBorderWidth);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        // draw inner circle
        canvas.drawCircle(mCenterPoint.x, mCenterPoint.y, mRadius + mBorderWidth / 2f, mInnerCircleBackgroundPaint);
        // draw ring
...
//        canvas.drawLine(mStopPoint.x, mStopPoint.y, mRightPoint.x, mRightPoint.y, mTickBackgroundPaint);
    ...
    private void startUnCheckedAnimation() {
        ...
        // 背景色渐变逻辑
        mBackgroundAnimator = ValueAnimator.ofArgb(mBackgroundColorChecked, mBackgroundColor);
        // 圆圈绘制完毕后后开始绘制
//        mBackgroundAnimator.setStartDelay((long) (mDuration * 0.23) + mDuration / 3);
        mBackgroundAnimator.setDuration((long) (mDuration * 0.5));
        mBackgroundAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                mInnerCircleBackgroundPaint.setColor(((int) animation.getAnimatedValue()));
                postInvalidate();
            }
        });
    ...
    private void startCheckedAnimation() {
        ...
        mCircleAnimator.setDuration((long) (mDuration * 0.25));
        mCircleAnimator.start();
        ...
        mLeftAnimator.setDuration((long) (mDuration * 0.14));
        mLeftAnimator.setInterpolator(new LinearInterpolator());
        mLeftAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
        ...
        mRightAnimator.setDuration((long) (mDuration * 0.2));
        mRightAnimator.setInterpolator(new LinearInterpolator());
        mRightAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
        ...
        // 背景色渐变逻辑
        mBackgroundAnimator = ValueAnimator.ofArgb(mBackgroundColor, mBackgroundColorChecked);
        // 勾的右边准备画完时开始画背景
        mBackgroundAnimator.setStartDelay((long) (mDuration * 0.3));
        mBackgroundAnimator.setDuration((long) (mDuration * 0.5));
        mBackgroundAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
